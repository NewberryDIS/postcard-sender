<script>
  ...
  const bannedFiles = ["2KXJ8ZSUDPJFW"];

  $: id = !bannedFiles.includes($page.params.id)
    ? $page.params.id
    : defaultPostcard;

  $: featuredPostcard = allPostcards
    .filter((f) => f.mei === id || f.image === id)
    .pop() || {
    gallery: "cats",
    title: "Cats",
    slug: "cats",
  ...
  $: currentGallery = allGalleries
    .filter((f) => f.slug === $activeGallery)
    .pop();
...
<main>
  <div class="content">
    <header>
      ...
      <p>
        Greet a friend with a vintage postcard image from the Newberry's
        collection
      </p>
    </header>
    <section class="image-section">
      <div class="asdf">
        ...
        {#if $showImage}
          <img
            src={imgUrl(featuredPostcard.image, "large")}
  ...
  <div class="gallery">
    <header>
      <h1>Newberry Postcard Sender</h1>
      <p>
        Greet a friend with a vintage postcard image from Newberry Digital
        Collections
      </p>
    </header>
